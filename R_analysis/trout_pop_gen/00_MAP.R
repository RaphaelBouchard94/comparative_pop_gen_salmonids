...
pop_trout<- read.table("~/Desktop/Baie_James_Paper/01_population_genomics/whitefish_pop_gen/pop_sampled.txt",header=T)

pop_sf <- st_as_sf(pop_trout, coords = c("long", "lat"), crs = 4326)

pop_sf$lat <- pop_trout$lat
...
ggplot() +
  geom_sf(data = world_geo, fill = "grey90")+
  geom_sf(data = pop_sf , 
          aes(fill = pop_code), color = "black",size = 5,pch = 21)+
  coord_sf(c(-81.75, -76), ylim = c(51.25, 54.75))+
  scale_fill_manual(values = paletteer_d(`"awtools::bpalette"`), name = "Population (n)")+
  annotate(geom = "text", 
           x = -80.7, y = 54, 
