...
pop_order_whitefish <- read.table("data/wgs_assign/whitefish/set_1_cocl_180_ind_eesbp_230697_pruned_singleton_snp.pop_names.txt")

#---> LOO

ind_id <- read.table("data/wgs_assign/whitefish/eesbp_samples.txt")

#List les fichiers LOO

files <- list.files(path = "~/Desktop/Baie_James_Paper/01_population_genomics/comparative_analysis/data/wgs_assign/whitefish/posterior_prob/", 
                    pattern = "LOO",
                    full.names = T)
...
#Add pop names
  colnames(setx) <- pop_order_whitefish$V1
...
post_prob_essbp <- as.data.frame(lapply(colnames(file_list_best_lik[[1]])[9], function(x) sapply(file_list_best_lik, `[[`, x))[[1]])
...
post_prob_essbp %<>% mutate(assigned = if_else(post_prob >= 0.6,"yes","no"))
...
#get id
non_essbp <- read.table("data/wgs_assign/whitefish/non_essbp_samples.txt")

#get files
files_assign <- list.files(path = "~/Desktop/Baie_James_Paper/01_population_genomics/comparative_analysis/data/wgs_assign/whitefish/posterior_prob/", 
...
#Merge the column origin of all element of the list into a single data.frame
post_prob_non_essbp <- as.data.frame(lapply(colnames(file_list_best_lik_assign[[1]])[9], function(x) sapply(file_list_best_lik_assign, `[[`, x))[[1]])
...
post_prob_non_essbp %<>% mutate(assigned = if_else(post_prob >= 0.6,"yes","no"))
...
whitefish_assignment <- rbind(post_prob_essbp,post_prob_non_essbp)

write.table(whitefish_assignment,"data/dispersal_model/whitefish_assignment.txt",quote = F,row.names = F)
